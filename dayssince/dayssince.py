...
class dayssince:
        ...
        async def on_reaction_add(reaction, user):
            ...
            if reaction.message.id == meltdown_message.id:
                if user == meltdown_user:
                    ...
                    await meltdown_message.edit(embed=new_embed)
    ...
    def make_embed(self, meltdown_user, meltdown_stats):
        ...
        meltdown_embed.description = "{} days ago.\n {} meltdowns in total".format(str(math.floor((time.time() - meltdown_user_stats["m"]) / 60 / 60 / 24)), str(meltdown_user_stats["c"])) if meltdown_user_stats else "No meltdowns!"
